...
## What it measures
For every kernel series `>= 6.12`, the latest patch release is analysed for its
Rust footprint:

- number of `*.rs` files and their on-disk size
- **purpose** of each file (kernel abstraction crate, procedural macros,
  generated/UAPI bindings, pin-init, core shims, samples, production drivers —
  broken out by subsystem — build tooling, other subsystems)
...
2. `scripts/render.py` — `data/kernels.json` → `index.html`

**Requirements:** `python3`, `curl`, `tar`, `xz`, `cloc`.
